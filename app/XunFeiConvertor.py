...
class XunFeiConvertor(RecordConvertor):
    ...
    def gene_request(self, apiname, data, files=None, headers=None):
        ...
        if result["ok"] == 0:
            # if format(apiname) == "/getResult":
            #     temp_data = result["data"]
            #     temp_data = self.replace_common_spoken_words(temp_data)
            #     json_data = json.loads(temp_data)
            #     fileName = self.getFileName() + ".txt"
            #     with open(fileName, 'w') as file_object:
            #         for content in json_data:
            #             file_object.write(content["onebest"])
            return result
        else:
    ...
    def replace_common_spoken_words(self, tempData):
        common_spoken_words = {"嗯", "呃", "啊", "哈", "就是", "这个", "那", "然后", "的话", "吧", "就", "呢", "噢", "哦"}
        for common_word in common_spoken_words:
            tempData = str(tempData).replace(common_word, "")
            tempData = str(tempData).replace("，，", "，")
            tempData = str(tempData).replace("。", "，")
        return tempData

    def getFileName(self):
        strlist = self.upload_file_path.split('.')
        fileName = strlist[0]
    ...
    def convert(self, upload_file_path):
        ...
        while True:
            ...
            time.sleep(5)
        # 5 . 获取结果
        return self.get_result_request(taskid=taskid)["data"]

# # 注意：如果出现requests模块报错："NoneType" object has no attribute 'read', 请尝试将requests模块更新到2.20.0或以上版本(本demo测试版本为2.20.0)
# # 输入讯飞开放平台的appid，secret_key和待转写的文件路径
# if __name__ == '__main__':
#     api = XunFeiConvertor(appid="fb0d46b1", secret_key="28d4a3f53e3a0386ecb941522f1335c8",
#                           upload_file_path=r"C:\\Users\\youlong\\Desktop\\9-13-东吴-隔膜行业专题.mp3")
#     api.all_api_request()
