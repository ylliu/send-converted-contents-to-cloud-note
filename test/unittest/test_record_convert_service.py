...
class RecordConvertServiceTest(unittest.TestCase):
    ...
    # def disabled_test_should_convert_multiple_records_and_send_out(self):
    #     # given
    #     # 拷贝多个文件到指定目录
    #     case_dir = "../test_audio/"
    #     files_to_be_copied = [case_dir + "1.mp3",
    ...
    #     sender = StubSender(self.config.token, self.config.namespace, self.config.format)
    #
    #     self.record_convert_service = RecordConvertService(record_convertor, sender, case_dir)
    #     self.record_convert_service.send_converted_contents_to_cloud_note()
    #
    #     # then
    #     # 问题是测试桩 StubSender 中间结果没保存的 只有返回值 外面不好断言
